...
public class MainActivity extends ListActivity {
	 ...
	 * //加载xml中的上下文菜单 MenuInflater inflater = getMenuInflater();
	 * inflater.inflate(R.menu.context, menu); super.onCreateContextMenu(menu,
	 * v, menuInfo); }
	 * 
	 * @Override public boolean onContextItemSelected(MenuItem item) {
	 * AdapterContextMenuInfo info = (AdapterContextMenuInfo)
	 * item.getMenuInfo(); // 相应上下文的操作 switch (item.getItemId()) { case
	 * R.id.edit: Toast.makeText(MainActivity.this, "Edit",
	 * Toast.LENGTH_LONG).show(); break; case R.id.share:
	 * Toast.makeText(MainActivity.this, "share", Toast.LENGTH_LONG).show();
	 * break; case R.id.delete: Toast.makeText(MainActivity.this, "delete",
	 * Toast.LENGTH_LONG).show(); break; default: break; } return
 ...
 * return false; }
 * 
 * @Override public void onDestroyActionMode(ActionMode mode) { // Here you can
 * make any necessary updates to the activity when // the CAB is removed. By
 * default, selected items are deselected/unchecked.
 * 
 * }
 * 
 ...
 */
/*
 * mode.setTitle("Select Items"); setActionModeTitle(mode); return true; }
 * 
 * 
 * @Override public boolean onActionItemClicked(ActionMode mode, MenuItem item)
 * { // Respond to clicks on the actions in the CAB switch (item.getItemId()) {
 * case R.id.menu_delete: deleteSelectedItems(); mode.finish(); // Action
 * picked, so close the CAB return true; case R.id.menu_cancel: // 全选 if
 * (mIsSelectAll) { item.setTitle("取消全选"); mIsSelectAll = false; } else {
 * item.setTitle("全选"); mIsSelectAll = true; } for (int i = 0; i <
 * mListView.getCount(); i++) { mListView.setItemChecked(i, !mIsSelectAll); }
 * return true;
 * 
 ...
 * 
 * @Override public void onItemCheckedStateChanged(ActionMode mode, int
 * position, long id, boolean checked) { // Here you can do something when items
 * are selected/de-selected, // such as update the title in the CAB
 * setActionModeTitle(mode);
 * 
 * } private void setActionModeTitle(ActionMode mode) { final int checkedCount =
 * getListView().getCheckedItemCount();
 * 
 * switch (checkedCount) { case 0: mode.setSubtitle(null); break; case 1:
 * mode.setSubtitle("1 item selected"); break; default:
 * mode.setSubtitle(checkedCount + " items selected"); break; } } });
 */